...
namespace Chat_Server.NetWork
{
    class ClientObject
    {
        ...
        stp_UserById_Result newUser;
        stp_UserByLogin_Result user;
                ...
                while (true)
                {
                    try
                    {
                        ...
                        else
                        {
                           ...
                           else if (str[0] == "Авторизация")
                            {    
                                
                                user = DataLayer._DL.ByLogin(str[1], str[2]);

                                if(user.FirstName != null)
                                server.RegistrMessage("1" + "\n" + user.FirstName, this.Id);
                                else
                                    server.RegistrMessage("0" + "\nnone", this.Id);
                                user = null;
                                //message += $" {newUser.Id}";
                            }
